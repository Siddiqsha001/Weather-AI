...
const Calendar = () => {
  ...
  const [notificationSettings, setNotificationSettings] = useState({
    reminderTimes: [30] //default 30 minutes
  });
  ...
  const getActivityStats = () => {
    ...
    setGoalStats(stats);   //map->const goals = { Running: 30, Reading: 20 };
                          //object.entries(goals) → [ ["Running", 30], ["Reading", 20] ]
  };
    ...
    for (const activity of upcomingActivities) {
      ...
      for (const minutesBefore of notificationSettings.reminderTimes) {
        ...
        if (!existingReminder) {
          ...
          //timeout for actual notification
          setTimeout(async () => {
  ...
  const handleUpdateActivity = async () => {
    try {
      const { data: { user } } = await supabase.auth.getUser();  //deconstructs to get user
